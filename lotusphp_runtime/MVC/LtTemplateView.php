...
namespace ARS\Lotus\MVC; echo \"\\r\";?>", $str);
        $str = str_replace("{LF}", "<?php

namespace ARS\Lotus\MVC; echo \"\\n\";?>", $str);
        // if else elseif
        $str = preg_replace("/\{if\s+(.+?)\}/", "<?php

namespace ARS\Lotus\MVC; if(\\1) { ?>", $str);
        $str = preg_replace("/\{else\}/", "<?php

namespace ARS\Lotus\MVC; } else { ?>", $str);
        $str = preg_replace("/\{elseif\s+(.+?)\}/", "<?php

namespace ARS\Lotus\MVC; } elseif (\\1) { ?>", $str);
        $str = preg_replace("/\{\/if\}/", "<?php

namespace ARS\Lotus\MVC; } ?>", $str);
        // loop
        $str = preg_replace("/\{loop\s+(\S+)\s+(\S+)\}/e", "\$this->addquote('<?php

namespace ARS\Lotus\MVC; if(isset(\\1) && is_array(\\1)) foreach(\\1 as \\2) { ?>')", $str);
        $str = preg_replace("/\{loop\s+(\S+)\s+(\S+)\s+(\S+)\}/e", "\$this->addquote('<?php

namespace ARS\Lotus\MVC; if(isset(\\1) && is_array(\\1)) foreach(\\1 as \\2=>\\3) { ?>')", $str);
...
namespace ARS\Lotus\MVC; echo LtObjectUtil::singleton('LtUrl')->generate(\\1);?>", $str);

        // 函数
        $str = preg_replace("/\{([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff:]*\s*\(([^{}]*)\))\}/", "<?php

namespace ARS\Lotus\MVC; echo \\1;?>", $str);
...
namespace ARS\Lotus\MVC; echo \\1['\\2'];?>", $str);
         */
        // 其它变量
        $str = preg_replace("/\{(\\$[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)\}/", "<?php
...
namespace ARS\Lotus\MVC; exit?>', '<?php

namespace ARS\Lotus\MVC; exit;?>'), array('', ''), $str);
        // 删除行首尾空白
        $str = preg_replace("/([\r\n]+)[\t ]+/s", "\\1", $str);
        $str = preg_replace("/[\t ]+([\r\n]+)/s", "\\1", $str);
        // 删除 {} 前后的 html 注释 <!--  -->
