...
class LtStoreFile implements LtStore
{
    ...
    public static $defaultStoreDir = "/tmp/LtStoreFile/";
    ...
    public function add($key, $value)
    {
        ...
        $length = file_put_contents($file, '<?php

namespace ARS\Lotus; exit;?>' . $value);
...
namespace ARS\Lotus; exit;?>' . $value);
    ...
    {
        $token = md5($key);
        return sprintf('%s%s/%s/%s/%s.php', $this->storeDir, $this->prefix, substr($token, 0, 2), substr($token, 2, 2), $token);
    }
}
