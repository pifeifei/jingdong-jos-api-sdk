...
class LtCaptchaImageEngine
{
    ...
    protected function ImageAllocate()
    {
        ...
        $this->im = imagecreatetruecolor(
            $this->conf['width'] * $this->conf['scale'],
            $this->conf['height'] * $this->conf['scale']
        );
        // Background color
    ...
    protected function WaveImage()
    {
        ...
        for ($i = 0; $i < ($this->conf['width'] * $this->conf['scale']); $i++) {
            imagecopy(
                ...
                $this->im,
                $i - 1,
                sin($k + $i / $xp) * ($this->conf['scale'] * $this->Xamplitude),
                $i,
                0,
                1,
                $this->conf['height'] * $this->conf['scale']
        ...
        for ($i = 0; $i < ($this->conf['height'] * $this->conf['scale']); $i++) {
            imagecopy(
                $this->im,
                $this->im,
                sin($k + $i / $yp) * ($this->conf['scale'] * $this->Yamplitude),
                $i - 1,
                0,
                $i,
                $this->conf['width'] * $this->conf['scale'],
