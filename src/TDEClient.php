...
final class TDEClient
{
    ...
    public const salt = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
    public const keyWordSalt = "\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C\x1D\x1E\x1F";
...
//        $this->mapper = new \JsonMapper();
    ...
    public static function getClientCache()
    {
        if (!self::$clientCache) {
            ...
            } elseif (extension_loaded('apcu') && 1 == ini_get('apc.enabled')) {
...
//    }
    ...
    public function decrypt($ct, $encoding = '')
    {
        ...
        if (ResultType::UnDecryptable === $cipherResult->status) {
        ...
        if (ResultType::Feasible === $cipherResult->status) {
        ...
        } elseif (ResultType::Malformed === $cipherResult->status) {
            $this->reporter->insertStatReport(StatisticType::DECERRCNT);
            // fetch available ciphertext
            $corrpted_cipher = '(NULL)';
            if (null !== $ct) {
                $min_len = min(strlen($ct), Constants::WEAK_HDR_LEN);
                $corrpted_cipher = 0 === $min_len ? '(EMPTY)' : base64_encode(substr($ct, 0, $min_len));
            }
            $this->reporter->insertErrReport(
    ...
    public function isEncryptionBytesData($ct)
    {
        try {
            ...
            $isStrong = false;
            if (
                Constants::CIPHER_TYPE_LARGE == $ctype
                || Constants::CIPHER_TYPE_REGULAR == $ctype
            ) {
                $isStrong = true;
            } elseif (Constants::CIPHER_TYPE_WEAK !== $ctype) {
                return false;
            }
    ...
    public function isDecryptable($ct)
    {
        try {
            ...
            if (null !== $this->cache_ks->searchDeckey($mkIdx)) {
                return ResultType::Decryptable;
            }
            if ($this->cache_ks->hasFutureKeyID($mkIdx)) {
    ...
    public function getCipherResult($ct)
    {
        try {
            ...
            if (
                CipherType::LARGE === CipherType::fromValue($ctype)
                || CipherType::REGULAR === CipherType::fromValue($ctype)
            ) {
                $flag = true;
            }
    ...
    public function calculateWildCardKeyWord($queryW, $asciiCharPrefixNumber = 0, $nonAsciiCharPrefixNumber = 0)
    {
        ...
        for ($i = 0; $i < mb_strlen($queryW); ++$i) {
            if (IndexCalculationHelper::ASCII_PLACEHOLDER == mb_substr($queryW, $i, 1)) {
                ++$skip;
            } else {
                break;
            }
    ...
    {
        return null == $this->token ? 'Unknown TID' : $this->token->get_id();
    }
    ...
    {
        return null == $this->token ? 'Unknown OriginType' : $this->token->getOriginType();
    }
...
//            }
            ...
            $this->statistic = [0, 0, 0, 0, 0, 0, 0, 0];
    ...
    private function check_key_status_forEncryption($key, $isEncryption = true)
    {
        ...
        if (KEY_STATUS::ACTIVE != $key->getKeyStatus()) {
        ...
        if (KEY_USAGE::N == $key->getKeyUsage() || KEY_USAGE::D == $key->getKeyUsage()) {
            if ($isEncryption) {
                $this->reporter->insertStatReport(StatisticType::ENCERRCNT);
            } else {
                $this->reporter->insertStatReport(StatisticType::SIGNERRCNT);
    ...
    private function check_key_status_forDecryption($key, $isDecryption = true)
    {
        ...
        if (KEY_STATUS::REVOKED === $key->getKeyStatus()) {
        ...
        if (KEY_USAGE::N === $key->getKeyUsage() || KEY_USAGE::E === $key->getKeyUsage()) {
            if ($isDecryption) {
                $this->reporter->insertStatReport(StatisticType::DECERRCNT);
            } else {
                $this->reporter->insertStatReport(StatisticType::VERIFYERRCNT);
...
{
    public const Decryptable = 0;
    public const Malformed = 1;
    public const Feasible = 2;
    public const UnDecryptable = 3;
}
...
{
    public const ENCCNT = 0;
    public const DECCNT = 1;
    public const ENCERRCNT = 2;
    public const DECERRCNT = 3;
    public const SIGNCNT = 4;
    public const VERIFYCNT = 5;
    public const SIGNERRCNT = 6;
    public const VERIFYERRCNT = 7;
