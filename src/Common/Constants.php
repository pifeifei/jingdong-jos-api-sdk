<?php

namespace ACES\Common;

use ACES\Common\Exception\MalformedException;

/**
 * ACES PHP Client constants
 * <P>.
 *
 * @author JD Data Security Team (tenma.lin, wei.gao, mozhiyan, xuyina)
 *
 * @version 1.0
 */
abstract class Constants
{
    public const CIPHER_TYPE_WEAK = 0;
    public const CIPHER_TYPE_REGULAR = 1;
    public const CIPHER_TYPE_LARGE = 2;
    public const CIPHER_TYPE_LEN = 1;

    public const ALGO_TYPE_AES_CBC_128 = 4;
    public const ALGO_TYPE_LEN = 1;

    public const DEFAULT_KEYID_LEN = 16;
    public const DEFAULT_DKEY_LEN = 16;
    public const DEFAULT_CIPHERBLK_LEN = 16;
    public const DEFAULT_SEED_LEN = 16;

    // http&https configuration
//    const PROD_CA_PATH = __DIR__ . "/cert/prod_ca_cert.crt";
    public const PROD_CA_RELATIVE_PATH = '/cert/prod_ca_cert.crt';
//    const BETA_CA_PATH = __DIR__ . "/cert/beta_ca_cert.crt";
    public const BETA_CA_RELATIVE_PATH = '/cert/beta_ca_cert.crt';
    public const HTTP_RETRY_MAX = 2;
    public const HTTP_TIMEOUT = 5000;     // 5000 ms

    public const TOKEN_EXP_DELTA = 2592000000;

    // algo
    public const DEFAULT_TOKEN_SIGN_ALGO = 'sha256';
    public const DEFAULT_CERTDIGEST_ALGO = 'sha256';

    // weak version header: CipherType(1b) Algorithm(1b) | MKeyID(16b)
    // 1 + 1 + 16
    public const WEAK_HDR_LEN = 18; // 18 fixed
    // strong version header:
    // cipher type, mkey_id length, mkey_id,
    // key cipher type, key cipher length, key cipher
    // data cipher type, data cipher length, data cipherh
    // 1 + 2 + 16 + 1 + 2 + 16 + 1 + 4
    public const STRONG_HDR_LEN = 43; // 43 fixed

    public const TMS_BTEA_TOKEN_CERT = '-----BEGIN CERTIFICATE----------END CERTIFICATE-----';

//    const TMS_PROD_TOKEN_CERT = "-----BEGIN CERTIFICATE----------END CERTIFICATE-----";
    public const TMS_PROD_TOKEN_CERT = "-----BEGIN CERTIFICATE-----\\n+3fYhR+QEhE8PboH5kVct1V9tiMpx7kymJQKVfNufC3FIlyr\n-----END CERTIFICATE-----";
    public const TDE_BETA_CA_CERT = '-----BEGIN CERTIFICATE----------END CERTIFICATE-----';

    public const TDE_PROD_CA_CERT = '-----BEGIN CERTIFICATE----------END CERTIFICATE-----';

    // index server configuration
    public const INDEX_SERVER_ENDPOINT = 'http://indexserver.aces.jd.local/kms/v1/host';
    public const DEV_INDEX_SERVER_ENDPOINT = 'http://indexsvr-indexserverdev.aces.svc.hcyf.n.jd.local/kms/v1/host';
    public const BETA_INDEX_SERVER_ENDPOINT = 'http://test.indexserver.aces.jd.local/kms/v1/host';

    // KMS configuration
    public const KMS_SERVER_ENDPOINT = 'https://kms.tde.jd.local';
    public const KMS_ENDPOINT_REQUEST_MK = '/request_mk';
    public const BETA_KMS_SERVER_ENDPOINT = 'https://beta.kms.tde.jd.local';
    public const DEV_KMS_SERVER_ENDPOINT = 'https://dev.kms.tde.jd.local';

    // ID
    public const ID_KMS_SERVER_ENDPOINT = 'https://kms.jd.id';
    public const ID_INDEX_SERVER_ENDPOINT = 'http://indexserver.aces.jd.id/kms/v1/host';
    // TH
    public const TH_KMS_SERVER_ENDPOINT = 'https://kms.jd.th.local';
    public const TH_INDEX_SERVER_ENDPOINT = 'http://indexserver.jd.th.local/kms/v1/host';
}

abstract class KEY_STATUS
{
    public const ACTIVE = 0;
    public const SUSPENDED = 1;
    public const REVOKED = 2;

    public static function fromValue($value)
    {
        if (null === $value) {
            throw new MalformedException('Key status is null.');
        }

        switch ($value) {
            case 0:
                return KEY_STATUS::ACTIVE;

            case 1:
                return KEY_STATUS::SUSPENDED;

            case 2:
                return KEY_STATUS::REVOKED;

            default:
                throw new MalformedException('Unknown key status.');
        }
    }
}

abstract class KEY_USAGE
{
    public const N = -1;
    public const E = 0;
    public const D = 1;
    public const ED = 2;

    public static function fromValue($value)
    {
        if (null === $value) {
            throw new MalformedException('Key usage is null.');
        }

        switch ($value) {
            case 'N':
                return KEY_USAGE::N;

            case 'E':
                return KEY_USAGE::E;

            case 'D':
                return KEY_USAGE::D;

            case 'ED':
                return KEY_USAGE::ED;

            default:
                throw new MalformedException('Unknown key usage.');
        }
    }
}

abstract class KEY_TYPE
{
    public const AES = 0;

    public static function fromValue($value)
    {
        if (null === $value) {
            throw new MalformedException('Key type is null.');
        }

        switch ($value) {
            case 0:
                return KEY_TYPE::AES;

            default:
                throw new MalformedException('Unknown key type.');
        }
    }
}
